...
def gaussian_noise(img, mu=-2, sigma=3, fre=1):
    ...
    r = RectBivariateSpline(mx, my, noise.T)
    f_noise = lambda mx, my: r(mx, my).T
...
# ------------------------------------------------------
# ------------- Functions added by Kai -----------------
# ------------------------------------------------------
def centralize_mesh_torch(pred_vertices):
    """
    Centralizes a 3D mesh using Numpy by calculating its bounding box
    and shifting its vertices to the origin.

    Args:
        pred_vertices (numpy array): A tensor of shape (N, 3), representing the vertices of the mesh.

    Returns:
        centralized_vertices (numpy array): The centralized vertices array.
    """
 
    # Compute the bounding box
    bbox_min = np.min(pred_vertices[:, 0:3], axis=0)
    bbox_max = np.max(pred_vertices[:, 0:3], axis=0)

    # Compute the center of the bounding box
    bbox_center = (bbox_min + bbox_max) / 2.0

    # Shift vertices to centralize the mesh
    centralized_vertices = pred_vertices.copy()
    centralized_vertices[:, 0:2] = pred_vertices[:, 0:2] - bbox_center[0:2]
    centralized_vertices[:, 2] = pred_vertices[:, 2] - bbox_min[2]

    # bbox_min = np.min(centralized_vertices, axis=0)
    # bbox_max = np.max(centralized_vertices, axis=0)
    # bbox_center = (bbox_min + bbox_max) / 2.0
    # print(f"box min: {bbox_min}, bbox_max: {bbox_max}, center {bbox_center}")
