...
def train(cfg: TrainPipelineConfig):
    ...
    """
    if distributed:
        # Get the local rank of the current process (used for multi-GPU training)
        # Default to 0 if LOCAL_RANK is not set in the environment variables
        local_rank = int(os.environ.get("LOCAL_RANK", 0))  
        
        # Set the current CUDA device to the local rank
        torch.cuda.set_device(local_rank)
        
        # Initialize the process group for distributed training using the NCCL backend
        # 'env://' specifies that initialization parameters are passed via environment variables
        torch.distributed.init_process_group(backend='nccl', init_method='env://')
        
        # Define the device for the current process (e.g., GPU corresponding to the local rank)
        device = torch.device("cuda", local_rank)

        # Print the local rank and device information for debugging purposes
        print(f"Local rank: {local_rank}")
        print(f"Device: {device}")
    else:
        device = torch.device(cfg.policy.device)  # Set the device for training (e.g., CPU or GPU)
    ...
    if distributed:
        ...
        print(f"Policy model loaded to DDP: {policy}")
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        batch_size=128, #cfg.batch_size,
        shuffle=shuffle,
        sampler=sampler,
        pin_memory=device.type != "cpu",
        drop_last=False,
    )
    logging.info(f"Dataloading success: {dataloader}")
    ...
    for _ in range(step, cfg.steps):
        ...
        train_tracker, output_dict = update_policy(
            ...
            # lock=torch.distributed.get_rank() if distributed else None,  # Lock for distributed training
        )
        ...
        if is_log_step and (not distributed or dist.get_rank() == 0):
        ...
        if cfg.save_checkpoint and is_saving_step and (not distributed or dist.get_rank() == 0):
        ...
        # Ensure all processes synchronize before moving to the next step
        if distributed:
            dist.barrier()
